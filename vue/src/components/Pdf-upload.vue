<template lang="html">
    <section>
        ...
        >
            <Button type="ghost" icon="ios-cloud-upload-outline" :loading="loading">上传文档</Button>
        </Upload>
        <div>
            <div v-if="preview">
    ...
    export default {
        ...
        data()
        {
            return {
                ...
                preview      : false,
                action       : (process.env.NODE_ENV == 'production' ? 'http://' + window.location.host : process.env.API_URL) + '/zupload/image',
        ...
        methods: {
            ...
            formatError(file)
            {
                this.$Notice.warning({
                    title: '文件格式不正确',
                    desc: '文件 ' + file.name + ' 格式不正确，只支持 pdf、txt、xlsx格式的文档。'
                });
            },
            maxSize(file)
            {
